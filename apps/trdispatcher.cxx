...
//      std::this_thread::sleep_until(startup_time + 2s);
      ...
      TLOG() << "TR Dispatcher " << config.my_id << ": " << "Configuring IOManager";
      config.configure_iomanager();
  
      auto publisher = std::make_unique<dunedaq::trdispatcher::PublisherTest>(config);
  
      for (size_t run = 0; run < config.num_runs; ++run) {
        TLOG() << "TR Dispatcher" << config.my_id << ": "<< "run " << run;
        if (config.num_apps>1)
          publisher->init(run);
        //publisher->send(run, forked_pids[0]);
        publisher->send(run, 0);
        TLOG() << "TR Dispatcher " << config.my_id << ": "<< "run " << run << " complete.";
      }
  
      TLOG() << "TR Dispatcher" << config.my_id << ": "<< "Cleaning up";
      publisher.reset(nullptr);
  
      dunedaq::iomanager::IOManager::get()->reset();
      TLOG() << "TR Dispatcher " << config.my_id << ": "<< "DONE";
