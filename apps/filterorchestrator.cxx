...
main(int argc, char* argv[])
{
    ...
    TLOG() << "Filter Orchestrator " << config.my_id << ": " << "Configuring IOManager";
    config.configure_iomanager();

    auto publisher = std::make_unique<dunedaq::filterorchestrator::PublisherTest>(config);

    for (size_t run = 0; run < config.num_runs; ++run) {
      TLOG() << "Filter Orchestrator " << config.my_id << ": "<< "run " << run;
      if (config.num_apps>1)
        publisher->init(run);
      //publisher->send(run, forked_pids[0]);
      publisher->send(run, 0);
      TLOG() << "Filter Orchestrator " << config.my_id << ": "<< "run " << run << " complete.";
    }

    TLOG() << "Filter Orchestrator" << config.my_id << ": "<< "Cleaning up";
    publisher.reset(nullptr);

    dunedaq::iomanager::IOManager::get()->reset();
    TLOG() << "Filter Orchestrator " << config.my_id << ": "<< "DONE";
