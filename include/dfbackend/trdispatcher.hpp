...
//      for (size_t sub = 0; sub < num_apps; ++sub) {
      for (size_t sub = 0; sub < 3; ++sub) {
        ...
        TLOG() << "Adding control connection " << "trdispatcher"+std::to_string(sub) << " with address "
                      << conn_addr;

        connections.emplace_back(
          //Connection{ ConnectionId{ "TR_tracking"+std::to_string(sub), "init_t" }, conn_addr, ConnectionType::kPubSub });
          Connection{ ConnectionId{ "trdispatcher"+std::to_string(sub), "init_t" }, conn_addr, ConnectionType::kSendRecv });
      }
...
{
    //TRDispatcher();
    const std::string storage_pathname = "/lcg/storage19/test-area/dune-v4-spack-integration2/sourcecode/daqconf/config/";
...
struct PublisherTest
{
    ...
    explicit PublisherTest(TRDispatcherConfig c)
      : config(c)
    {
    }
  ...
  void init(size_t run_number){
    ...
    auto init_sender = dunedaq::get_iom_sender<dunedaq::datafilter::Handshake>("trdispatcher0");
    auto init_receiver = dunedaq::get_iom_receiver<dunedaq::datafilter::Handshake>("trdispatcher1");
  ...
  void send(size_t run_number, pid_t subscriber_pid)
  {
    ...
    auto init_receiver = dunedaq::get_iom_receiver<dunedaq::datafilter::Handshake>("trdispatcher2");
