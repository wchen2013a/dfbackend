...
struct HDF5FromStorage
{
    ...
    HDF5FromStorage(const std::string &storage_pathname, const std::string json_file)
    ...
    void HDF5Find()
    {
        ...
        for (auto const& entry : std::filesystem::directory_iterator{daq_storage_path})
        {
           ...
           if (file_ext == ".hdf5") 
           {
               ...
               for (auto item : hdf5_files_already_transfer )
               {
                   ...
                   if (item != entry.path().filename().string()){
                        std::cout << "transfer" << entry.path().filename().string() << '\n';
        ...
        hdf5_files_to_transfer.erase(unique(hdf5_files_to_transfer.begin(),hdf5_files_to_transfer.end()),hdf5_files_to_transfer.end());
    ...
    void ReadWriteJSON()
    {
        ...
        std::sort(hdf5_files_already_transfer.begin(),hdf5_files_already_transfer.end());
        hdf5_files_already_transfer.erase(unique(hdf5_files_already_transfer.begin(),hdf5_files_already_transfer.end()),hdf5_files_already_transfer.end());
        save(json_file);
    }
